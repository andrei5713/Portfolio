...
const skills = [
  ...
  { name: "VB .NET", level: 70, category: "frontend" },

  // Backend
  { name: "Laravel", level: 60, category: "backend" },
  { name: "MySQL", level: 70, category: "backend" },
  { name: "PostgreSQL", level: 70, category: "backend" },
  { name: "Kotlin", level: 20, category: "backend" },
  { name: "MongoDB", level: 60, category: "backend" },
  { name: "C#", level: 70, category: "backend" },
  { name: "PHP", level: 50, category: "backend" },
  { name: "Ruby", level: 10, category: "backend" },
      ...
      <div className="container mx-auto max-w-5xl">
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-12">
          {categories.map((category, key) => (
            <button key={key} onClick={() => setActiveCategory(category)} className={cn("px-5 py-2 rounded-full transition-colors duration-300 capitalize", activeCategory === category ? "bg-primary text-primary-foreground" : "bg-secondary/70 text-forefround hover:bd-secondary")}>
              {category}
            </button>
          ))}
        </div>
